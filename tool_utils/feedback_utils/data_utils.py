...
class Tokenizer4LLM:
    def __init__(self, max_seq_len, pretrained_model, llm=True):
        if llm:
            ...
            if 'GPT' in pretrained_model or "llama" in pretrained_model or "/gemini/pretrain" in pretrained_model:
                # 让[pad]填充标记变为[eos]结束标记
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.run_pre:
            ...
            for example in examples_with_template:
                #input_prompt += self.get_input_template(example, self.dataset_name) + f"Label: {example['sentiment']}"+"\n\n"
                input_prompt += self.get_input_template(example,
                                                        self.dataset_name) + f"Label: {example['label']}" + "\n\n"
            ...
            input_prompt = self.prompt
            for example in examples_with_template:
                #input_prompt += self.get_input_template(example, self.dataset_name) + f"Label: {example['quadruples'][0]['sentiment']}"+"\n\n"
                input_prompt += self.get_input_template(example,
                                                        self.dataset_name) + f"Label: {example['label']}" + "\n\n"
            input_tokens = input_prompt + \
                self.get_input_template(data, self.dataset_name) + "Label: "  
        elif self.cfg.run_ficl:
            example_id = self.example_ids[index] # 获取相关性样本average_ranks_per_input_sample.json的id
            # get TopK examples by id in the train dataset
            examples = []
            preds, truth = [], []
            for i in range(self.n_example):
                if i < self.cfg.wrong_exps:
                    for num, j in enumerate(example_id):
                        ...
                        if self.examples.iloc[j]["sentiment"] != self.examples.iloc[j]["prediction"]:
                            examples_with_template.append(dict(self.examples.iloc[j]))
                            examples.append(j)
                            preds.append(self.examples.iloc[j]['prediction'])
                            truth.append(self.examples.iloc[j]['sentiment'])
                            break
                else:
                    for j in example_id:
                        if j in examples:
                        ...
                        if self.examples.iloc[j]["sentiment"] == self.examples.iloc[j]["prediction"]:
                            examples_with_template.append(dict(self.examples.iloc[j]))
                            examples.append(j)
                            preds.append(self.examples.iloc[j]['sentiment'])
                            truth.append(self.examples.iloc[j]['sentiment'])
                            break
            input_prompt = self.prompt
            ...
            for ii, example in enumerate(examples_with_template):
                if preds[ii] != truth[ii]:
                    input_prompt += f"{self.get_input_template(example, self.dataset_name)}" + f"Prediction: {preds[ii]}\n"\
                         + f"Correct Label: {example['sentiment']}\n" + self.feedback_prompt["wrong"]
                else:
                    input_prompt += f"{self.get_input_template(example, self.dataset_name)}" + f"Prediction: {example['sentiment']}\n"\
...
class ClassificationDataset_SemEvallap14(Dataset):
    # Dataset for In-Context Learning
    # both for Openai API and Local LLMs
    def __init__(self, data, cfg, instruct_prompt, get_template, all_labels=None, tokenizer=None, icl_mode=None,
                 examples=None, example_ids=None, feedback_prompt=None, thinking_prompt=None):
    ...
    def __getitem__(self, index):
        data = dict(self.data.iloc[index])
        tokens = data['sentence']
        if not data.get('aspects') or len(data['aspects'][0]) == 0:
            data['aspects'] = [{'term':'UNKNOWN','polarity': 'UNKNOWN'}]
        if not data['aspects'][0]:
            data['aspects'] = [{'term':'UNKNOWN','polarity': 'UNKNOWN'}]
        aspects = data['aspects']
        label = aspects[0]['polarity']
        examples_with_template = []
        # input tokens ()
        if self.cfg.run_pre:
            examples = []
            ...
            for example in examples_with_template:
                input_prompt += self.get_input_template(example,
                                                        self.dataset_name) + f"Label: {example['aspects'][0]['polarity']}" + "\n\n"
            input_tokens = input_prompt + self.get_input_template(data, self.dataset_name) + "Label: "
        elif self.cfg.run_icl:
            # get ICL examples
            example_id = self.example_ids[index]  # get corresponding example id
            ...
            input_prompt = self.prompt
            for example in examples_with_template:
                if not example.get('aspects') or len(example['aspects'][0]) == 0:
                    example['aspects']= [{'term':'UNKNOWN','polarity': 'UNKNOWN'}]
                if not example['aspects'][0] :
                    example['aspects'] = [{'term':'UNKNOWN','polarity': 'UNKNOWN'}]
                input_prompt += self.get_input_template(example,
                                                        self.dataset_name) + f"Label: {example['aspects'][0]['polarity']}" + "\n\n"
                print(input_prompt)
            ...
            for ii, example in enumerate(examples_with_template):
                if preds[ii] != truth[ii]:
                    ...
                    input_prompt += f"{self.get_input_template(example, self.dataset_name)}" + f"Prediction: {example['sentiment']}\n" \
                                    + f"Correct Label: {example['sentiment']}\n" + self.feedback_prompt["correct"]
            # pred = data['prediction']
            input_tokens = input_prompt + self.get_input_template(data,
                                                                  self.dataset_name) + "Correct Label: "  # self.thinking_prompt
