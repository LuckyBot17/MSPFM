...
from src.acos_MultiPerspectives import BertForSequence, trainMulti, evaluateMulti
...
def handle_MultiPerspectives():
    ...
    bert_model = BertForSequence.from_pretrained(model_path,tokenizer=bert_tokenizer, max_seq_length=args.max_seq_length,
                                                 shot_num=args.shot_num,set_num=args.set_num,device=[0,1],batch_size=args.batch_size)
    ...
    train_pt_text = evaluateMulti(bert_model,train_examples,batch_size=args.batch_size,out_dir=f"./MultiPerspectives_BERT/{task}/{dataset}/trains_{train_idx}_text.pt",flag='text')
    test_pt_text = evaluateMulti(bert_model,test_examples,batch_size=args.batch_size,out_dir=f"./MultiPerspectives_BERT/{task}/{dataset}/tests_{train_idx}_text.pt",flag='text')
    ...
    train_pt_label = evaluateMulti(bert_model, train_examples, batch_size=args.batch_size,out_dir=f"./MultiPerspectives_BERT/{task}/{dataset}/trains_{train_idx}_label.pt",flag='label')
    test_pt_label = evaluateMulti(bert_model, test_examples, batch_size=args.batch_size,out_dir=f"./MultiPerspectives_BERT/{task}/{dataset}/tests_{train_idx}_label.pt", flag='label')
    ...
    train_pt_pos = evaluateMulti(bert_model, train_examples, batch_size=args.batch_size,out_dir=f"./MultiPerspectives_BERT/{task}/{dataset}/trains_{train_idx}_pos.pt",flag='pos')
    test_pt_pos = evaluateMulti(bert_model, test_examples, batch_size=args.batch_size,out_dir=f"./MultiPerspectives_BERT/{task}/{dataset}/tests_{train_idx}_pos.pt", flag='pos')
...
def Averarge_Ranking3Perspectives(text_path,label_path,pos_path):
    ...
    for input_sample in range(818):
        ...
        # 保存到新的JSON文件
        output_file_path = f'./ICL_examples/ATSC/{args.dataset}/average_ranks_per_input_sample.json'
...
def retrieve_toJson(args,similarity_matrix,perspective):
    ...
    writeJson(f"./ICL_examples/{args.task}/{args.dataset}/MultiPerspectives_{args.train_idx}_{perspective}.json", result_json, encoding="utf-8")
...
if __name__ == '__main__':
    ...
    Averarge_Ranking3Perspectives(
        f"./ICL_examples/{args.task}/{args.dataset}/MultiPerspectives_{args.train_idx}_text.json",
        f"./ICL_examples/{args.task}/{args.dataset}/MultiPerspectives_{args.train_idx}_label.json",
        f"./ICL_examples/{args.task}/{args.dataset}/MultiPerspectives_{args.train_idx}_pos.json")
